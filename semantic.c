...
void methodop(tree node)
{
  int nStrInd;     /* String Table index */
  int nSymInd;     /* Symbol Table index */
  tree child, child1, child2, child3, child4, child5, child6;
  int count=0;
	...
	nStrInd = IntVal(LeftChild(child));
        if( nStrInd == loc_str("main") )
	  {
		...
		if( GetAttr(i, NAME_ATTR)== nStrInd )
		  {
		    error_msg(MULTI_MAIN, CONTINUE, nStrInd, 0);
		    return;
		  }
	  }
    ...
    // child2 = (tree)LeftChild(child);
	  // if (! IsNull(child2))
	  // {
		  // printf("Hello\n");
		  // child3 = (tree)RightChild(child2);
		  // child4 = (tree)LeftChild(child3);
		 // if( (NodeOp(child4) == VArgTypeOp) || ( NodeOp(child4) == RArgTypeOp ) )
		 // {
			 // printf("Hello again\n");
			 // count++;                                
	         // child5 = (tree)RightChild(child4);
	         // if( (NodeOp(child5) == VArgTypeOp) || ( NodeOp(child5) == RArgTypeOp ) )
	  	        // count++;                                 
         // } 
// //	  SetAttr( nSymInd, ARGNUM_ATTR, count );
	  // }
    // SetAttr( nSymInd, ARGNUM_ATTR, count );
